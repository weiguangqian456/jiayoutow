...
/**
 *
 * ClassName:      AllOrderFragment
 * <p>
 * Author:
 * <p>
 * CreateDate:      2020/9/24 11:12
 * <p>
 * Description:   全部订单
 */

class AllOrderFragment : BaseMvpFragment() {
    // 请求数据
    private var Inform_Target: PublicPresenter? = null

    var ordeRecycler: SwipeRecyclerView? = null
    ...
    override val layoutId: Int
        get() = R.layout.order_all_fragment

    override fun initView(view: View?, savedInstanceState: Bundle?) {
        Inform_Target = PublicPresenter(context, true, "加载中")
        Inform_Target?.attachView(this)
        ordeRecycler = view?.findViewById(R.id.rv_all_order)
        mErrorView =view?.findViewById(R.id.cev_view)
        val layoutParams = mErrorView!!.layoutParams as MarginLayoutParams
        ...
        ordeRecycler?.adapter = baseRecyclerAdapter
        newData()
        ordeRecycler?.useDefaultLoadMore()
        ordeRecycler?.loadMoreFinish(false, true)
        ordeRecycler?.setLoadMoreListener {
            mHttpPage++
            newData()
        }
    }
    ...
    fun newData() {
        Inform_Target?.netWorkRequestGet(HttpURL.OrderZhuBangquery, HttpRequest.OrderRefurlist(MyApplication.MOBILE, "10", mHttpPage.toString(), "8.3.11", ""))
